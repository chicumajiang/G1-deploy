...
class FasByd7s(FSMState):
    def __init__(self, state_cmd:StateAndCmd, policy_output:PolicyOutput):
        super().__init__()
        self.state_cmd = state_cmd
        self.policy_output = policy_output
        self.name = FSMStateName.SKILL_FAS_BYD_7S
        self.name_str = "fas_byd_7s"
        self.motion_phase = 0
        self.counter_step = 0
        self.ref_motion_phase = 0
        
        current_dir = os.path.dirname(os.path.abspath(__file__))
        config_path = os.path.join(current_dir, "config", "FasByd7s.yaml")
    ...
    def checkChange(self):
        # Check if motion is complete (7 seconds)
        motion_time = self.counter_step * 0.02
        if motion_time >= 7.0:
